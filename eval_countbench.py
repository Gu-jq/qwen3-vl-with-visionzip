...
def main():
    # 1. 自动加载 CountBenchQA 数据集
    print(f"正在准备 CountBenchQA 数据集...")
    dataset = build_dataset('CountBenchQA')
    data = dataset.data
    ...
    for i in tqdm(indices, desc="评测中"):
        item = data.iloc[i]
        # dataset.build_prompt 会自动添加 "Note that: answer with a number directly..." 提示词
        ...
        # 5. 评分逻辑
        # CountBenchQA 官方逻辑是判断答案字符串是否在预测字符串中
        # 为了更准确，我们这里使用提取数字后比对的逻辑，同时保留 vlmeval 的包含判断作为 fallback
        pred = extract_number(response)
        gt = str(item['answer'])
        
        if pred == gt or gt in response:
            correct += 1
        total += 1
    ...
    print("\n" + "="*40)
    print(f"CountBench 评测报告 ({'压缩版' if USE_COMPRESSION else '原始版'})")
    print(f"总样本数: {total}")
